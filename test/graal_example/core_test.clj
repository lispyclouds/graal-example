...
(ns graal-example.core-test
  (:require [clojure.test :refer :all]
            [graal-example.core :refer :all]))

(deftest fetch-test
  (testing "Fetching stock for a symbol"
    (let [result (get-stock-value-of "MSFT")]
      (is (and (instance? String (result :name))
               (number? (result :price)))))))
